...
class assign_feedback_essayscorer extends assign_feedback_plugin {
    ...
    public function get_name() {
        return get_string('pluginname', 'assignfeedback_essayscorer');
    }
    ...
    public function get_quickgrading_html($userid, $grade) {
        ...
        $pluginname = get_string('pluginname', 'assignfeedback_essayscorer');
    ...
    public function get_editor_fields() {
        return array('comments' => get_string('pluginname', 'assignfeedback_essayscorer'));
    }
    ...
    public function get_form_elements_for_user($grade, MoodleQuickForm $mform, stdClass $data, $userid) {
        ...
        // Retrieve the current assignment description
        $assignmentinstance = $this->assignment->get_instance();
        $instruction = strip_tags($assignmentinstance->intro);

        // Store the image file submission path
        $imagefilepath = ''; // Initialize the variable
        $context = $this->assignment->get_context();

        foreach ($this->assignment->get_submission_plugins() as $plugin) {
            if ($plugin->is_enabled() && $plugin->is_visible() && !$plugin->is_empty($submission)) {
                $user = $DB->get_record('user', ['id' => $submission->userid]);
                // Check if the submission plugin is for file submission
                if ($plugin instanceof assign_submission_file) {
                    // Get the files from the submission
                    if ($files = $plugin->get_files($submission, $user)) {
                        $fs = get_file_storage();
                        foreach ($files as $file) {
                            if ($file instanceof stored_file && $this->is_image_file($file->get_filename())) {

                                // $fileimage = base64_encode(file_get_contents($CFG->dataroot . '/' . $file->get_pathnamehash()));
                                // $fileimage = $CFG->dataroot . '/' . $file->get_contenthash();
                                // // $fileimage = $file->get_content();
                                // $imagefilepath .= 'test';
                                ...
                                // Get the file content directly from Moodle's file storage
                                $fileContent = $file->get_content();    

                                // Define the custom directory path within the dataroot folder
                                $customDirectory = $CFG->dataroot . '/essayscorer_submissions/';
                                ...
                                $filePathInCustomDirectory = $customDirectory . $file->get_filename();
                                
                                // Save the file content to the custom directory
                                file_put_contents($filePathInCustomDirectory, $fileContent);
                                // $file->copy_content_to("{$requestdir}/helloworld.txt");

                                $imagefilepath = $filePathInCustomDirectory;

                                // $contentHash = $file->get_contenthash();
                                // $filePath = $CFG->dataroot . '/filedir/' . substr($contentHash, 0, 2) . '/' . substr($contentHash, 2, 2) . '/' . $contentHash;
                                // $imagefilepath .= $filePath;
                                // Assuming only one file is submitted, break after getting the file path
                                break;
        ...
        $mform->addElement('button', 'assignfeedbackessayscorer_generatescore', get_string('generate_score', 'assignfeedback_essayscorer'));
        $mform->addElement('editor', 'assignfeedbackcomments_editor', $this->get_name(), null, $this->get_editor_options());

        // Call JavaScript module
        $PAGE->requires->js_call_amd('assignfeedback_essayscorer/feedbackgenerate', 'init', [[
     ...
     * @return bool
     */
    protected function is_image_file($filename) {
        $image_extensions = ['jpe','jpg', 'jpeg', 'png'];
